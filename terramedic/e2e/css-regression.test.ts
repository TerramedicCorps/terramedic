...
test.describe('Navbar responsive layout', () => {
  test('nav links should be visible and inline on desktop', async ({ page }) => {
    ...
    // About dropdown trigger should be visible (it's a button inside NavLi)
    const aboutTrigger = nav.getByRole('button', { name: 'About menu' });
  ...
  test('nav links should be hidden behind hamburger below lg breakpoint', async ({ page }) => {
    ...
    const aboutTrigger = page.locator('nav').getByRole('button', { name: 'About menu' });
    await expect(aboutTrigger).not.toBeVisible();
