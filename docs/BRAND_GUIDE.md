...
**Urgent but hopeful.** The planet needs help now, but
there are real things people can do.

**Accessible, not academic.** Write for someone who cares
about the planet but doesn't have a background in
environmental science.

**Action-oriented, not guilt-driven.** Lead with what
people can do, not what they've done wrong.

**Warm, empowering, practical.** The reader should feel
invited, not lectured.

**Inclusive.** Focus on what unites people — a shared
desire for a healthier world — not on identity labels
like "environmentalist." The message is for everyone who
cares about the planet, not a specific in-group.
...
- **terramedic** (all lowercase) — the wordmark and brand
  mark
- **Terramedic** (capital T) — in prose and sentence
  context
- **Terramedic Corps** — the full organization name
- **Terramedic Corps, Inc.** — the legal entity name (use
  in copyright notices, privacy policy, and legal docs
  only)
- **"Earth Guardian"** — the established visual design
  system (dark, bold, space-themed)

### Word Choices

- **Avoid "climate" in user-facing copy.** The word can be
  polarizing. Use "environmental," "the planet," "the
  Earth," "a healthier world" instead. Exception: SDG 13
  is officially named "Climate Action" and can be
  referenced as such.
- **"Heal the planet"** is the core invitation. It opens
  the door to anyone who cares, not just people who
  already identify as environmentalists.
- **"Emergency Responders for the Planet"** — used as a
  hero tagline, but use with care. Avoid contexts where
  it could be read as equating the org with actual first
  responders or EMTs. Body copy should use softer framing:
  "turning awareness into action," "matching people with
  opportunities."
- **"Environmental organizations"** — not "climate orgs"
  in user-facing copy.
- **SDG references** — always spell out on first use
  ("UN Sustainable Development Goal 13 (Climate Action)"),
  then abbreviate ("SDG 13").
- **The green cross logo** — signals environmental first
  aid. Reference it this way, not as "the plus sign" or
  "the icon."
...
- Always provide a call to action. Every page should
  answer: "what can I do right now?"
- No jargon. No doom framing. No identity gatekeeping.
- Write for someone scanning on a phone. Front-load the
  important information.
- When describing the platform's AI capabilities, follow
  the framing guidelines in
  [AGENTS.md](../AGENTS.md#human-ai-framing).

### The Test

Before publishing any user-facing text, ask:

> "If a volunteer or nonprofit leader read this, would
> they feel valued and empowered — or sidelined and
> lectured?"

If the answer isn't clearly the first, revise.

---

## Brand Overview

Terramedic is an environmental action platform connecting
people with volunteer opportunities, donation options, and
daily sustainability practices. The "Earth Guardian" design
system evokes the overview effect — seeing Earth from space
— to inspire urgency and stewardship.

**Design philosophy**: Dark backgrounds convey the vastness
of space; bright accent colors (green, blue, gold)
represent hope and action. Every color pairing must meet
...
A bold green cross — like the Red Cross, but green. Signals
...
Cross on a white circle background, for use at small sizes
in browser tabs.
...
Maintain a minimum clear space of 25% of the logo diameter
on all sides (e.g., 10px clear space around a 40px logo).
...
- Recolor the cross to anything other than green
...
Darker shades of each accent color, used exclusively for
button backgrounds so that white text passes WCAG AA
(4.5:1+). The lighter primary accent colors remain for text
links, card accents, the logo, and other decorative uses.
...
Minimum ratios: **4.5:1** for normal text, **3:1** for
...
*White on Terra Blue passes for large bold text only
(3.7:1 > 3:1).
...
- **Buttons** (green, blue, gold): Use `text-white` on
  darker button-shade backgrounds (`--btn-green`,
  `--btn-blue`, `--btn-gold`)
- **Labels & section headings**: Use `text-gray-400` on
  any dark background
- **Error messages**: Use `text-red-400` on any dark
  background
- **Success messages**: Use `text-green-400` on any dark
  background
- **Body text**: Use `text-white` or `text-gray-400` on
  dark backgrounds
- **Links in footer**: Use `text-gray-400` with
  `hover:text-white`

### Don't

- Never use `text-white` on the lighter accent colors
  (`#2ecc71`, `#2196f3`, `#f39c12`) — only on darker
  button shades
- Never use `text-gray-500` on dark backgrounds (Space
  Black, Deep Navy, Navy)
- Never use `text-red-500` on dark backgrounds — use
  `text-red-400` instead
- Never place low-contrast text-secondary (`#b0bec5`) at
  sizes below 14px without checking ratio
...
| Element | Color | Class / Style |
|---|---|---|
| Background | Radial ellipse to black | `.hero-space-bg` gradient |
...
| Links | Gray 400 to White hover | `text-gray-400 hover:text-white` |
...
Used as a thin accent strip below the navbar and above the
footer. Represents the progression from cooler (blue) to
warmer (red) years in global temperature data.
...
CSS variable (extended version):
`--warming-stripes-gradient`

### Hero Video Background

A looping, muted video of Earth from space fills the hero
section. A dark scrim gradient overlays the video for text
legibility.

- **Video sources**: `static/videos/earth-hero.webm`, `.mp4`
- **Poster image**: `static/images/earth-hero-poster.jpg`
- **Scrim**: Linear gradient from 80% black (top) to
  transparent (bottom)
...
All color tokens are defined in `src/app.css` under
`:root`:
...
Custom colors are registered via the `@theme` block in
`src/app.css` (Tailwind v4 does **not** read
...
Use as Tailwind classes: `bg-terra-green`, `bg-navy`,
`bg-btn-green`, etc.

Button text uses `text-white` on the darker `btn-*`
backgrounds for WCAG AA compliance. The lighter accent
colors (`terra-green`, `terra-blue`, `sunrise-gold`) are
for links, card accents, and the logo.

### Font Loading

Fonts are self-hosted as woff2 files in `static/fonts/`
(latin subset):

- **Inter** (variable, 400-700): Body text, UI elements
  (`font-sans`)
